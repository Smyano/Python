
# ANOTAÇÕES
# nome = (input("Digite seu nome: "))
# idade = (input("Digite sua idade: "))
# print(f"Nome: {nome} e Idade: {idade} anos")

# print(f"Nome: {nome:^10}")


# n1 = int(input("Digite o primeiro número: "))
# n2 = int(input("Digite o segundo número: "))    
# soma = n1 + n2
# print(f"A soma de {n1} e {n2} é: {soma}")

# nome = input("Digite seu nome: ")
# print (f"olá" , nome)

# dia = input("Digite o dia do seu nascimento: ")
# mes = input("Digite o mes do seu nascimento: ")
# ano = input("Digite o ano do seu nascimento: ")
# print(f"Você nasceu no dia {dia} de {mes} de {ano}")

# n1 = int(input("Digite um número:"))
# n2 = int(input("Digite um outro:"))
# soma = n1 + n2
# print(f"A soma dos núnmeros digitados são: {soma}") 
# == igual

# tipos primitivos: int, string, boolean, float

# operadores aritméticos
# ** potência, // divisão interia, %Rdivisão

# ordem de precendência
# 1° (); 
# 2° ** "** > potência"; **1/2> raiz quadrada 
# 3° *, /, //, %;
# 4° +, -;

# biblioteca import MACH:
# from math import " # , # " (importar somente o tipo especifico)
# ceil (arredonda +)
# floor (arredonda -)
# trunc (sem arredondamento)
# pow(potência)
# sqrt(raiz quadrada)
# factorial (fatorial "!")

# aula 8:
# import math
# num = int(input("digite um número "))
# raiz = math.sqrt(num) 
# a = math.floor(raiz)
# print (f"a raiz quadrada de {num} são {a}")

# .limit_denominator() usado para import fraction


# [] > serve para criar listas

# (aula 10):
# tempo = int(input("Quanto tempo tem seu carro? "))
# if tempo <=3:
#     print("carro novo")
# else: 
#     print("carro velho")
#     print ("fim")

# condição simplificada:
#     tempo = int(input"quanto tempo tem seu carro?")
# print("carro novo" if tempo <=3 else "carro velho")
# print ("fim")

# aula 13, estrutura de repetição for
# for c in range(0,6):
#     print("Olá")
# print("Tchau")
# for c in range(6,0, -1): #contagem regrassiva
    # print(c)
    
# n = int(input("Digite um número: "))
# for c in range(0,n):
#     print(n)
    
# n = int(input("Digite um número: "))
# for c in range(n,0, -1):
#     print(c)

# i = int(input("Digite o número do início: "))
# f = int(input("Digite o número final: "))
# m = int(input("Digite o número intermédiario: "))
# for p in range(i,f,m):
#     print(p)
# s = 0 #guarda o valor do primeiro item digitado
# for c in range(0,3):
#     n = int(input("Digite um número: "))
#     s+=n
# print(f"A soma dos números é {s}")

n = int(input("Digue sua idade: "))
menor_de_idade = False

if n >= 18:
    maior_de_idade = True

if menor_de_idade == False:
    print("Você é menor de idade")